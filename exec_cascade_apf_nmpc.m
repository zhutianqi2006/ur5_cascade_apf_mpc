...
high_Ts = 0.4;
Ts = 0.04;
high_mpc_matrix_gain = eye(8);
aobs_stop_flag = 0;
bobs_stop_flag = 0;
for i = 1:7
    high_mpc_matrix_gain(i,i) = 2000;
end
high_mpc_matrix_gain(8,8) = 1.5;
...
for i=1:550
    ...
    if(mod(i,5)==1)
        ...
        current_states(32) = robotdetJacobianTemp;
        ref_energy =[ref(1:7) 0.0];
        dis_aobj2ee =power((power(ee_xyz(1)-aMovObs.Pose(1,4),2)+power(ee_xyz(2)-aMovObs.Pose(2,4),2)+power(ee_xyz(3)-aMovObs.Pose(3,4),2)),1/2);
        dis_aobj2p1 =power((power(p1(1,4)-aMovObs.Pose(1,4),2)+power(p1(2,4)-aMovObs.Pose(2,4),2)+power(p1(3,4)-aMovObs.Pose(3,4),2)),1/2);
        dis_aobj2p2 =power((power(p2(1,4)-aMovObs.Pose(1,4),2)+power(p2(2,4)-aMovObs.Pose(2,4),2)+power(p2(3,4)-aMovObs.Pose(3,4),2)),1/2);
        dis_bobj2ee =power((power(ee_xyz(1)-bMovObs.Pose(1,4),2)+power(ee_xyz(2)-bMovObs.Pose(2,4),2)+power(ee_xyz(3)-bMovObs.Pose(3,4),2)),1/2);
        dis_bobj2p1 =power((power(p1(1,4)-bMovObs.Pose(1,4),2)+power(p1(2,4)-bMovObs.Pose(2,4),2)+power(p1(3,4)-bMovObs.Pose(3,4),2)),1/2);
        dis_bobj2p2 =power((power(p2(1,4)-bMovObs.Pose(1,4),2)+power(p2(2,4)-bMovObs.Pose(2,4),2)+power(p2(3,4)-bMovObs.Pose(3,4),2)),1/2);
        dis_obj_flag = dis_aobj2ee>0.3&&dis_aobj2p1>0.3&&dis_aobj2p2>0.3&&dis_bobj2ee>0.3&&dis_bobj2p1>0.3&&dis_bobj2p2>0.3;
        vel_bobj_flag = abs(current_states(28))<0.05&&abs(current_states(29))<0.05&&abs(current_states(30))<0.05;
        vel_aobj_flag = abs(current_states(22))<0.05&&abs(current_states(23))<0.05&&abs(current_states(24))<0.05;
        too_far_flag = dis_aobj2ee>0.4&&dis_aobj2p1>0.4&&dis_aobj2p2>0.4&&dis_bobj2ee>0.4&&dis_bobj2p1>0.4&&dis_bobj2p2>0.4;
        if((dis_obj_flag&&vel_bobj_flag&&vel_aobj_flag)||too_far_flag)
            for j = 1:3
                high_mpc_matrix_gain(j,j) = 1.5;
            end
            for j = 4:7
                high_mpc_matrix_gain(j,j) = 0;
            end
            high_mpc_matrix_gain(8,8) = 0;
        else
            for j = 1:3
                high_mpc_matrix_gain(j,j) = 2000;
            end
            for j = 4:7
                high_mpc_matrix_gain(j,j) = 0;
            end
        ...
        high_out = high_apf_nmpc_RUN(0.0,ref_energy,current_states(1,1:31),...
           J_ee(1,1),J_ee(1,2),J_ee(1,3),J_ee(1,4),J_ee(1,5),J_ee(1,6), ...
           J_ee(2,1),J_ee(2,2),J_ee(2,3),J_ee(2,4),J_ee(2,5),J_ee(2,6), ...
           J_ee(3,1),J_ee(3,2),J_ee(3,3),J_ee(3,4),J_ee(3,5),J_ee(3,6), ...
           J_ee(4,1),J_ee(4,2),J_ee(4,3),J_ee(4,4),J_ee(4,5),J_ee(4,6), ...
           ...
           J_p2(2,1),J_p2(2,2),J_p2(2,3),J_p2(2,4),J_p2(2,5),J_p2(2,6), ...
           J_p2(3,1),J_p2(3,2),J_p2(3,3),J_p2(3,4),J_p2(3,5),J_p2(3,6),...
           high_mpc_matrix_gain);
         
        high_joint_pos = [joint_pos(1)+high_Ts*high_out.U(1);joint_pos(2)+high_Ts*high_out.U(2);joint_pos(3)+high_Ts*high_out.U(3);joint_pos(4)+high_Ts*high_out.U(4);joint_pos(5)+high_Ts*high_out.U(5);joint_pos(6)+high_Ts*high_out.U(6)];
        high_tform = getTransform(matlab_UR5, high_joint_pos,'ee_link');
        high_ee_quat = rotm2quat(high_tform(1:3,1:3));
        high_ee_xyz = high_tform(1:3,4)';
        high_ref = [high_ee_xyz,ref(4:13)];
    end
    %% Get translation matrix of end-effecotr and other collision check points
    tform = getTransform(matlab_UR5, joint_pos,'ee_link');
    p1 = getTransform(matlab_UR5, joint_pos,'forearm_link');
    ...
    current_states(31) = (bMovObs.Pose(3,4)-last_bMovObs.Pose(3,4))/0.04;
    current_states(32) = robotdetJacobianTemp;
   out = low_nmpc_RUN(0.0,high_ref,current_states, ...
    J_ee(1,1),J_ee(1,2),J_ee(1,3),J_ee(1,4),J_ee(1,5),J_ee(1,6), ...
    J_ee(2,1),J_ee(2,2),J_ee(2,3),J_ee(2,4),J_ee(2,5),J_ee(2,6), ...
    J_ee(3,1),J_ee(3,2),J_ee(3,3),J_ee(3,4),J_ee(3,5),J_ee(3,6), ...
    J_ee(4,1),J_ee(4,2),J_ee(4,3),J_ee(4,4),J_ee(4,5),J_ee(4,6), ...
    ...
    high_ref_x_seq(i) =ref(1)- high_ref(1);
    i_sequence(i) = Ts*i;
end
