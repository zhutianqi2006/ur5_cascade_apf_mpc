...
BEGIN_ACADO;
acadoSet('problemname', 'low_nmpc'); %mex file name
% states
...
det_jq1 = acado.MexInput;
det_jq2 = acado.MexInput;
det_jq3 = acado.MexInput;
det_jq4 = acado.MexInput;
det_jq5 = acado.MexInput;
det_jq6 = acado.MexInput;
...
f.add(dot(det_val) == det_jq1*joint1_v+det_jq2*joint2_v+det_jq3*joint3_v+det_jq4*joint4_v+det_jq5*joint5_v+det_jq6*joint6_v);
% define the predict: t_start default is zero
...
h={ee_px ee_py ee_pz ee_r1 ee_r2 ee_r3 ee_r4 joint1_v joint2_v joint3_v joint4_v joint5_v joint6_v};
...
ocp.subjectTo( 0.15 <=power(power(p1_px-object1x,2)+power(p1_py-object1y,2)+power(p1_pz-object1z,2),1/2) );
ocp.subjectTo( 0.15 <=power(power(p2_px-object1x,2)+power(p2_py-object1y,2)+power(p2_pz-object1z,2),1/2) );
ocp.subjectTo( 0.15 <=power(power(ee_px-object1x,2)+power(ee_py-object1y,2)+power(ee_pz-object1z,2),1/2) );
ocp.subjectTo( 0.15 <=power(power(p1_px-object2x,2)+power(p1_py-object2y,2)+power(p1_pz-object2z,2),1/2) );
ocp.subjectTo( 0.15 <=power(power(p2_px-object2x,2)+power(p2_py-object2y,2)+power(p2_pz-object2z,2),1/2) );
ocp.subjectTo( 0.15 <=power(power(ee_px-object2x,2)+power(ee_py-object2y,2)+power(ee_pz-object2z,2),1/2) );
ocp.subjectTo( 1e-14<=power(det_val,2));
% Solving method and final setup
algo = acado.RealTimeAlgorithm(ocp,0.04);
